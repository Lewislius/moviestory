...
# Default parameters
#!/bin/bash
# Batch generation script for I2V with multi-GPU support
# This script processes all example folders (001-010) and generates videos using 4 GPUs
...
echo "========================================"
echo "Starting batch I2V generation"
...
for i in $(seq $start_num $end_num); do
    ...
    echo "[$folder_name] Using $NUM_GPUS GPUs"
    echo ""
    
    # Run the generation command with torchrun
    echo "[$folder_name] Starting multi-GPU generation..."
    start_time=$(date +%s)
    
    torchrun --nproc_per_node=$NUM_GPUS ${work_dir}/model/Wan2.2/generate.py \
        --task i2v-A14B \
        --size "$SIZE" \
    ...
    #         --nproc_per_node 4 \
    #         ${work_dir}/model/Wan2.2/generate.py \
    #         --task i2v-A14B \
    #         --ckpt_dir "$CHECKPOINT_DIR" \
    #         --image "$ref_image" \
    #         --refert_num 1 \
...
echo ""
echo "========================================"
echo "Batch I2V generation completed!"
echo "========================================"
