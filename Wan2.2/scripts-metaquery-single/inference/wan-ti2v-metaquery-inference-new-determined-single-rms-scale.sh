...
INFER_ENTRY="${INFER_ENTRY:-${SCRIPT_DIR}/inference_metaquery_wan_rms_aligned.py}"
...
run_mq_case() {
  ...
  local cmd=(
    ...
    --mq_norm_log_interval "${MQ_NORM_LOG_INTERVAL}"
    --shift "${SHIFT}"
  ...
  if [[ "${MQ_NORM_MATCH_EVERY_STEP}" == "1" ]]; then
    cmd+=(--mq_norm_match_every_step)
  fi
  if [[ "${MQ_NORM_LOG_EACH_STEP}" == "1" ]]; then
...
echo "[DEMO] infer_entry=${INFER_ENTRY}"
echo "[DEMO] cfg_uncond_mode=${CFG_UNCOND_MODE}"
echo "[DEMO] mq_norm_probe_with_t5=${MQ_NORM_PROBE_WITH_T5} mq_norm_match_t5=${MQ_NORM_MATCH_T5}"
echo "[DEMO] mq_norm_warn_ratio=[${MQ_NORM_WARN_RATIO_LOW}, ${MQ_NORM_WARN_RATIO_HIGH}]"
echo "[DEMO] mq_norm_match_clip=[${MQ_NORM_MATCH_CLIP_MIN}, ${MQ_NORM_MATCH_CLIP_MAX}]"
echo "[DEMO] mq_norm_match_every_step=${MQ_NORM_MATCH_EVERY_STEP} mq_norm_log_each_step=${MQ_NORM_LOG_EACH_STEP} log_interval=${MQ_NORM_LOG_INTERVAL}"
