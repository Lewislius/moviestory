...
# checkpoints
CHECKPOINT_PATH="${CHECKPOINT_PATH:-/home/liuzhirui/model/Wan2.2/checkpoint/metaquery_overfit20/ti2v_overfit30_steps2000_nummq256_nullimg0.0_nullcap0.0/checkpoint-final}"
...
VERIFY_FAIL_ON_WARNING="${VERIFY_FAIL_ON_WARNING:-0}"
...
# animate ref-slot args
ANIMATE_REFSLOT_SEGMENT_FRAMES="${ANIMATE_REFSLOT_SEGMENT_FRAMES:-78}"
ANIMATE_REFSLOT_REF_FRAMES="${ANIMATE_REFSLOT_REF_FRAMES:-1}"
ANIMATE_REFSLOT_TEMPORAL_FRAMES="${ANIMATE_REFSLOT_TEMPORAL_FRAMES:-0}"
ANIMATE_REFSLOT_CONDITIONAL_FRAMES="${ANIMATE_REFSLOT_CONDITIONAL_FRAMES:-0}"
ANIMATE_REFSLOT_PRESERVE_REINJECT="${ANIMATE_REFSLOT_PRESERVE_REINJECT:-1}"

OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/outputs_overfit10_animate_like_v2_20260325}"
mkdir -p "${OUTPUT_DIR}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-animate_like_v2}"
...
run_case() {
  local case_name="$1"
  local mode="$2"
  local i2v_method="$3" # none | legacy_ref_lock | animate_ref_slot
  local use_ref="$4"    # 1 | 0
  ...
  local cmd=(
    "${PYTHON_BIN}" "${SCRIPT_DIR}/inference_metaquery_wan_animate_like_v2.py"
  ...
  if [[ "${mode}" == "i2v" && "${i2v_method}" == "legacy_ref_lock" ]]; then
    cmd+=(--i2v_method legacy_ref_lock)
    cmd+=(--i2v_ref_strategy "${I2V_REF_STRATEGY}")
  fi

  if [[ "${mode}" == "i2v" && "${i2v_method}" == "animate_ref_slot" ]]; then
    cmd+=(--i2v_method animate_ref_slot)
    cmd+=(--animate_refslot_segment_frames "${ANIMATE_REFSLOT_SEGMENT_FRAMES}")
    cmd+=(--animate_refslot_ref_frames "${ANIMATE_REFSLOT_REF_FRAMES}")
    cmd+=(--animate_refslot_temporal_frames "${ANIMATE_REFSLOT_TEMPORAL_FRAMES}")
    cmd+=(--animate_refslot_conditional_frames "${ANIMATE_REFSLOT_CONDITIONAL_FRAMES}")
    if [[ "${ANIMATE_REFSLOT_PRESERVE_REINJECT}" == "1" ]]; then
      cmd+=(--animate_refslot_preserve_reinject)
    else
      cmd+=(--animate_refslot_no_preserve_reinject)
    fi
  fi

  echo "[RUN] case=${case_name} mode=${mode} i2v_method=${i2v_method} use_ref=${use_ref}"
...
echo "[RUN] verify_level=${VERIFY_LEVEL}"
echo "[RUN] verify_train_before_checkpoint=${VERIFY_TRAIN_BEFORE_CHECKPOINT:-<none>}"
echo "[RUN] 将生成 4 个视频:"
echo "  1) t2v: ref 仅进 MQ"
echo "  2) t2v: no_ref（ref 不进 MQ，不进 Wan）"
echo "  3) i2v + legacy_ref_lock: ref 进 MQ + Wan 首帧条件"
echo "  4) i2v + animate_ref_slot: ref 作为 prefix slot"
...
# 1) t2v: ref 只进 MQ，不进 Wan 首帧条件
run_case "mq_t2v_mq_ref_only" "t2v" "none" "1"

# 2) t2v: ref 不进 MQ，不进 Wan（纯文本）
run_case "mq_t2v_no_ref_text_only" "t2v" "none" "0"

# 3) i2v + legacy_ref_lock
run_case "mq_i2v_legacy_ref_lock" "i2v" "legacy_ref_lock" "1"

# 4) i2v + animate_ref_slot
run_case "mq_i2v_animate_ref_slot" "i2v" "animate_ref_slot" "1"

run_chain_audit

echo "[DONE] 4 个 case 已完成，结果在 ${OUTPUT_DIR}"
...
# # 这个也是，上面是t5 + mq作为条件的版本，这个是只mq作为条件的版本
# set -euo pipefail
...
# # checkpoints
# CHECKPOINT_PATH="${CHECKPOINT_PATH:-/home/liuzhirui/model/Wan2.2/checkpoint/metaquery_overfit10_animate_like_v2/ti2v_overfit10_steps_mq_only500/checkpoint-500}"
# WAN_CHECKPOINT_DIR="${WAN_CHECKPOINT_DIR:-/home/liuzhirui/model/Wan2.2/Wan2.2-TI2V-5B}"
# QWEN3VL_MODEL_ID="${QWEN3VL_MODEL_ID:-/home/liuzhirui/model/Qwen3-VL-main/Qwen3-VL-2B-Thinking}"
...
# OFFLOAD_MODEL="${OFFLOAD_MODEL:-1}"
# I2V_FORCE_SIZE="${I2V_FORCE_SIZE:-1}"
# VERIFY_LEVEL="${VERIFY_LEVEL:-basic}"
# VERIFY_FAIL_ON_WARNING="${VERIFY_FAIL_ON_WARNING:-0}"
# CHECKPOINT_LAYOUT_STRICT="${CHECKPOINT_LAYOUT_STRICT:-1}" # 1 => 关键文件缺失直接退出
# VERIFY_TRAIN_BEFORE_CHECKPOINT="${VERIFY_TRAIN_BEFORE_CHECKPOINT:-}" # 训练前基线checkpoint(可选)
# CHAIN_AUDIT_EPS="${CHAIN_AUDIT_EPS:-1e-7}"
...
# OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/outputs_overfit10_animate_like_v2}"
# mkdir -p "${OUTPUT_DIR}"
# RUN_TS="${RUN_TS:-$(date +%Y%m%d-%H%M%S)}"
# OUTPUT_PREFIX="${OUTPUT_PREFIX:-animate_like_v2}"
...
# echo "[RUN] verify_level=${VERIFY_LEVEL}"
# echo "[RUN] verify_train_before_checkpoint=${VERIFY_TRAIN_BEFORE_CHECKPOINT:-<none>}"
# echo "[RUN] dit_condition_mode=${DIT_CONDITION_MODE}"
# echo "[RUN] 将生成 4 个视频（均为 MQ-only DiT 注入）:"
# echo "  1) t2v: ref 仅进 MQ"
# echo "  2) t2v: no_ref（ref 不进 MQ）"
# echo "  3) i2v + legacy_ref_lock: 接口兼容，实际回退 MQ-only 路径"
# echo "  4) i2v + animate_ref_slot: 接口兼容，实际回退 MQ-only 路径"
