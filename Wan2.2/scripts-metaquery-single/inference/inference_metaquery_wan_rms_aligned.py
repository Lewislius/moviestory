...
#         action="store_true",
#         default=True,
#         help="推理时对比 MQ 与 T5 的 token RMS 范数，并写入 verify report。",
#     )
#     p.add_argument(
#         "--disable_mq_norm_probe_with_t5",
#         action="store_false",
#         dest="mq_norm_probe_with_t5",
#         help="关闭 MQ vs T5 范数探针。",
#     )
#     p.add_argument(
#         "--mq_norm_warn_ratio_low",
#         type=float,
#         default=0.25,
#         help="当 MQ/T5 RMS 比值低于该阈值时给出告警。",
#     )
#     p.add_argument(
#         "--mq_norm_warn_ratio_high",
#         type=float,
#         default=4.0,
#         help="当 MQ/T5 RMS 比值高于该阈值时给出告警。",
#     )
#     p.add_argument(
#         "--mq_norm_match_t5",
#         action="store_true",
#         default=True,
#         help="将 MQ(cond/uncond) 按 token RMS 对齐到 T5 的 RMS（默认开启，和训练侧一致）。",
#     )
#     p.add_argument(
#         "--disable_mq_norm_match_t5",
#         action="store_false",
#         dest="mq_norm_match_t5",
#         help="关闭 MQ(cond/uncond) 对齐到 T5 RMS。",
#     )
#     p.add_argument(
#         "--mq_norm_match_clip_min",
#         type=float,
#         default=0.03,
#         help="mq_norm_match_t5 时，缩放因子下限。",
#     )
#     p.add_argument(
#         "--mq_norm_match_clip_max",
#         type=float,
#         default=4.0,
#         help="mq_norm_match_t5 时，缩放因子上限。",
#     )
#     p.add_argument(
#         "--mq_norm_match_every_step",
#         action="store_true",
#         default=False,
#         help="在去噪每一步都重新执行一次 MQ->T5 RMS 对齐（用于显式证明每步均缩放）。",
#     )
#     p.add_argument(
#         "--mq_norm_log_each_step",
#         action="store_true",
#         default=False,
#         help="打印每一步 MQ RMS 缩放日志（含缩放前后 RMS 与 scale）。",
#     )
#     p.add_argument(
#         "--mq_norm_log_interval",
#         type=int,
#         default=1,
#         help="mq_norm_log_each_step 开启时的打印步长（1=每步打印）。",
#     )
...
#         action="store_true",
#         default=True,
#         help="若 checkpoint 中存在 wan_dit_trainable.*，则加载到 Wan DiT（默认开启）。",
#     )
#     p.add_argument(
...
#         if picked.suffix.lower() == ".safetensors":
#             try:
#                 from safetensors.torch import load_file
#                 state = load_file(str(picked), device="cpu")
#             except Exception as e:
#                 raise RuntimeError(f"加载 Wan safetensors 失败: {picked}") from e
#         else:
#             payload = _safe_torch_load(picked, map_location="cpu")
#             if not isinstance(payload, dict):
#                 raise RuntimeError(f"Wan trainable 文件格式异常: {picked}")
...
#         if wan_mode == "full" and missing:
#             self._warn(
#                 f"训练记录 wan_train_mode=full，但推理加载出现 missing keys({len(missing)})，"
#                 "这通常表示 Wan 权重键名不兼容（例如 FSDP flat key）。"
#             )
#         self._record_runtime_metric("wan_finetune_loaded", 1)
#         self._record_runtime_metric("wan_finetune_state_tensors", int(len(state)))
#         self._record_runtime_metric("wan_finetune_missing", int(len(missing)))
#         self._record_runtime_metric("wan_finetune_unexpected", int(len(unexpected)))
#         self._record_runtime_metric("wan_finetune_path", str(picked))
...
#     def _compute_t5_rms_for_mq_norm(self, prompt: str, tag: str) -> float:
#         t5_runtime_device = self._resolve_t5_runtime_device()
#         self._record_runtime_metric(f"{tag}_t5_runtime_device", str(t5_runtime_device))
#         with torch.no_grad():
#             t5_ctx = self.wan.text_encoder([prompt], t5_runtime_device)[0].to(
#                 self.device, dtype=torch.bfloat16
#             )
#         t5_rms = self._token_rms(t5_ctx)
#         self._record_runtime_metric(f"{tag}_t5_rms_probe", t5_rms)
#         return float(t5_rms)

#     def _scale_mq_to_target_rms(
#         self,
#         feat: torch.Tensor,
#         target_rms: float,
#     ) -> tuple[torch.Tensor, float, float, float]:
#         before_rms = self._token_rms(feat)
#         if target_rms <= 0:
#             return feat, before_rms, before_rms, 1.0
#         smin = float(getattr(self.args, "mq_norm_match_clip_min", 0.03))
#         smax = float(getattr(self.args, "mq_norm_match_clip_max", 4.0))
#         scale = float(target_rms / (before_rms + 1e-8))
#         scale = float(max(smin, min(smax, scale)))
#         out = feat * scale
#         after_rms = self._token_rms(out)
#         return out, before_rms, after_rms, scale

#     def _maybe_apply_stepwise_mq_match(
#         self,
#         mq_feat_cond_base: torch.Tensor,
#         mq_feat_uncond_base: torch.Tensor,
#         target_t5_rms: float,
#         tag: str,
#         step_idx: int,
#         total_steps: int,
#     ) -> tuple[torch.Tensor, torch.Tensor]:
#         match_every_step = bool(getattr(self.args, "mq_norm_match_every_step", False))
#         if not match_every_step:
#             return mq_feat_cond_base, mq_feat_uncond_base

#         if not bool(getattr(self.args, "mq_norm_match_t5", True)):
#             if step_idx == 0:
#                 print(f"[MQ-RMS][{tag}] mq_norm_match_every_step=1 但 mq_norm_match_t5=0，跳过逐步缩放。")
#             return mq_feat_cond_base, mq_feat_uncond_base

#         cond_scaled, cond_before, cond_after, cond_scale = self._scale_mq_to_target_rms(
#             mq_feat_cond_base, target_t5_rms
#         )
#         uncond_scaled, uncond_before, uncond_after, uncond_scale = self._scale_mq_to_target_rms(
#             mq_feat_uncond_base, target_t5_rms
#         )

#         log_each_step = bool(getattr(self.args, "mq_norm_log_each_step", False))
#         log_interval = max(1, int(getattr(self.args, "mq_norm_log_interval", 1)))
#         if log_each_step and (step_idx % log_interval == 0 or step_idx == total_steps - 1):
#             print(
#                 f"[MQ-RMS][{tag}] step={step_idx + 1}/{total_steps} "
#                 f"t5_rms={target_t5_rms:.6f} "
#                 f"cond: {cond_before:.6f}->{cond_after:.6f} (scale={cond_scale:.6f}) "
#                 f"uncond: {uncond_before:.6f}->{uncond_after:.6f} (scale={uncond_scale:.6f})"
#             )

#         return cond_scaled, uncond_scaled

#     def _resolve_t5_runtime_device(self) -> torch.device:
#         """
#         根据 Wan text_encoder 当前参数所在设备，动态决定文本编码设备。
#         这样可兼容:
#         - T5 常驻 CUDA
#         - T5 常驻 CPU
#         - 运行时被 offload 到 CPU 的情况
#         """
#         t5_model = getattr(getattr(self.wan, "text_encoder", None), "model", None)
#         if isinstance(t5_model, nn.Module):
#             try:
#                 return next(t5_model.parameters()).device
#             except StopIteration:
#                 pass
#             except Exception:
#                 pass
#             try:
#                 return next(t5_model.buffers()).device
#             except StopIteration:
#                 pass
#             except Exception:
#                 pass
...
#     def _probe_and_match_mq_norm(
#         self,
#         prompt: str,
#         mq_feat_cond: torch.Tensor,
#         mq_feat_uncond: torch.Tensor,
#         tag: str,
#     ) -> tuple[torch.Tensor, torch.Tensor]:
#         probe = bool(getattr(self.args, "mq_norm_probe_with_t5", True))
#         match = bool(getattr(self.args, "mq_norm_match_t5", True))
#         if not (probe or match):
#             return mq_feat_cond, mq_feat_uncond

#         def _probe_warn(msg: str) -> None:
#             # 范数探针只做“观测+对齐”，不应因告警中断主生成流程。
#             self.verify_report["warnings"].append(msg)
#             print(f"[VERIFY][WARN] {msg}")

#         try:
#             t5_rms = self._compute_t5_rms_for_mq_norm(prompt, tag)
#             cond_rms = self._token_rms(mq_feat_cond)
#             uncond_rms = self._token_rms(mq_feat_uncond)
#             ratio = cond_rms / (t5_rms + 1e-8)

#             self._record_runtime_metric(f"{tag}_mq_rms", cond_rms)
#             self._record_runtime_metric(f"{tag}_mq_uncond_rms", uncond_rms)
#             self._record_runtime_metric(f"{tag}_t5_rms_probe", t5_rms)
#             self._record_runtime_metric(f"{tag}_mq_to_t5_rms_ratio", ratio)

#             low = float(getattr(self.args, "mq_norm_warn_ratio_low", 0.25))
#             high = float(getattr(self.args, "mq_norm_warn_ratio_high", 4.0))
#             if ratio < low or ratio > high:
#                 self._record_runtime_metric(f"{tag}_mq_to_t5_ratio_out_of_range", 1)
#                 _probe_warn(
#                     f"{tag} MQ/T5 RMS 比值异常: ratio={ratio:.4f} (mq={cond_rms:.4f}, t5={t5_rms:.4f}, range=[{low}, {high}])"
#                 )
#             else:
#                 self._record_runtime_metric(f"{tag}_mq_to_t5_ratio_out_of_range", 0)

#             if match and t5_rms > 0:
#                 mq_feat_cond, _, _, s_cond = self._scale_mq_to_target_rms(mq_feat_cond, t5_rms)
#                 mq_feat_uncond, _, _, s_uncond = self._scale_mq_to_target_rms(mq_feat_uncond, t5_rms)
#                 self._record_runtime_metric(f"{tag}_mq_norm_match_enabled", 1)
#                 self._record_runtime_metric(f"{tag}_mq_norm_match_scale_cond", s_cond)
#                 self._record_runtime_metric(f"{tag}_mq_norm_match_scale_uncond", s_uncond)
#         except Exception as e:
#             self._record_runtime_metric(f"{tag}_mq_norm_probe_failed", 1)
#             _probe_warn(f"{tag} MQ/T5 范数探针失败: {e}")
...
#         else:
#             mq_feat_null = torch.zeros_like(mq_feat)
#         mq_feat_effective = mq_feat
#         mq_feat_effective, mq_feat_null = self._probe_and_match_mq_norm(
#             prompt=prompt,
#             mq_feat_cond=mq_feat_effective,
#             mq_feat_uncond=mq_feat_null,
#             tag="t2v",
#         )
#         t5_rms_target = float(self.verify_report["runtime"].get("t2v_t5_rms_probe", 0.0))
#         if bool(getattr(self.args, "mq_norm_match_every_step", False)) and t5_rms_target <= 0:
#             try:
#                 t5_rms_target = self._compute_t5_rms_for_mq_norm(prompt, "t2v")
#             except Exception as e:
#                 self._warn(f"t2v 逐步 MQ RMS 对齐预计算失败: {e}")
#                 t5_rms_target = 0.0
#         self._record_runtime_metric("t2v_mq_norm_match_every_step", int(bool(getattr(self.args, "mq_norm_match_every_step", False))))
#         self._record_runtime_metric("t2v_mq_norm_log_each_step", int(bool(getattr(self.args, "mq_norm_log_each_step", False))))
#         self._record_runtime_metric("t2v_mq_norm_target_t5_rms_for_step", float(t5_rms_target))
...
#                 step_mq_cond, step_mq_uncond = self._maybe_apply_stepwise_mq_match(
#                     mq_feat_cond_base=mq_feat_effective,
#                     mq_feat_uncond_base=mq_feat_null,
#                     target_t5_rms=t5_rms_target,
#                     tag="t2v",
#                     step_idx=step_idx,
#                     total_steps=len(timesteps),
#                 )
#                 aug_context = [step_mq_cond]
#                 aug_null = [step_mq_uncond]
...
# 下面这个是为了适配那个fsdp训练下参数不匹配的请=情况：此时是可以适配首帧绑定的最终生成效果的！
# """
# inference_metaquery_wan.py
# ==========================
# MetaQuery + Wan2.2 TI2V 推理脚本。
...
#                    help="文本描述")
#     p.add_argument("--ref_image", type=str, default=None,
#                    help="参考图路径（用于 MetaQuery 编码；i2v 模式下用于首帧绑定）")
#     p.add_argument("--negative_prompt", type=str, default="",
#                    help="负面提示词")
...
#         help=(
#             "i2v 显式条件方式: "
#             "legacy_ref_lock=首帧 latent 锚定；"
#             "animate_ref_slot=当前文件不支持，将自动回退到 legacy_ref_lock"
#         ),
#     )
#     p.add_argument("--frame_num", type=int, default=81,
#                    help="生成帧数 (4n+1)")
...
#         help=(
#             "i2v 首帧注入策略: "
#             "animate_like=前期强锚定后期释放；"
#             "hard_lock=每步强锚定并在输出阶段精确替换首帧"
#         ),
#     )
#     p.add_argument("--max_area", type=int, default=480 * 832,
#                    help="最大面积")
...
#         """
#         legacy_ref_lock i2v：
#         1) WAN context 仍然仅使用 MQ 特征；
#         2) 参考图首帧会编码成 latent，并在去噪中反复锚定第一帧；
#         3) hard_lock 模式下最终首帧像素也会被精确替换为参考图。
#         """
#         from wan.utils.fm_solvers import (
#             FlowDPMSolverMultistepScheduler,
#             get_sampling_sigmas,
...
#         device = self.device
#         strategy = str(getattr(self.args, "i2v_ref_strategy", "hard_lock")).strip().lower()
#         if strategy not in {"animate_like", "hard_lock"}:
#             strategy = "hard_lock"
#         use_cfg = self._use_cfg(guide_scale)
#         cfg_uncond_mode = str(getattr(self.args, "cfg_uncond_mode", "encode_negative")).strip().lower()
#         if cfg_uncond_mode not in {"encode_negative", "zero_mq"}:
...
#         self._record_runtime_metric("i2v_method", "legacy_ref_lock")
#         self._record_runtime_metric("i2v_mode_effective", f"legacy_ref_lock_{strategy}")
#         self._record_runtime_metric("i2v_ref_explicit_in_dit", 1)
#         self._record_runtime_metric("i2v_text_explicit_in_dit", 0)
#         self._record_runtime_metric("i2v_legacy_ref_lock_use_cfg", int(use_cfg))
#         self._record_runtime_metric("i2v_legacy_ref_lock_strategy", strategy)
#         self._record_runtime_metric("i2v_legacy_ref_lock_guide_scale", float(guide_scale))
#         self._record_runtime_metric("i2v_legacy_ref_lock_cfg_uncond_mode", cfg_uncond_mode)
...
#             ow, oh = best_output_size(iw, ih, dw, dh, max_area)
#         print(
#             f"[Generate][i2v][legacy_ref_lock] output_size={ow}x{oh} "
#             f"(input_ref={iw}x{ih}, force_size={self.args.i2v_force_size}, "
#             f"max_area={max_area}, strategy={strategy})"
#         )

#         print("[Generate][legacy_ref_lock] MetaQuery 编码...")
...
#             mq_feat_cond=mq_feat_effective,
#             mq_feat_uncond=mq_feat_null,
#             tag="i2v_legacy_ref_lock",
#         )
#         t5_rms_target = float(self.verify_report["runtime"].get("i2v_legacy_ref_lock_t5_rms_probe", 0.0))
#         if bool(getattr(self.args, "mq_norm_match_every_step", False)) and t5_rms_target <= 0:
#             try:
#                 t5_rms_target = self._compute_t5_rms_for_mq_norm(prompt, "i2v_legacy_ref_lock")
#             except Exception as e:
#                 self._warn(f"i2v_legacy_ref_lock 逐步 MQ RMS 对齐预计算失败: {e}")
#                 t5_rms_target = 0.0
#         self._record_runtime_metric(
#             "i2v_legacy_ref_lock_mq_norm_match_every_step",
#             int(bool(getattr(self.args, "mq_norm_match_every_step", False))),
#         )
#         self._record_runtime_metric(
#             "i2v_legacy_ref_lock_mq_norm_log_each_step",
#             int(bool(getattr(self.args, "mq_norm_log_each_step", False))),
#         )
#         self._record_runtime_metric(
#             "i2v_legacy_ref_lock_mq_norm_target_t5_rms_for_step",
#             float(t5_rms_target),
#         )
#         self._verify_mq_feature_sensitivity(
#             prompt=prompt,
...
#         aug_context = [mq_feat_effective]
#         aug_null = [mq_feat_null]
#         self._verify_context_plugged(mq_feat_effective, aug_context[0], tag="i2v_legacy_ref_lock")
...
#         ref_tensor = TF.to_tensor(img_cropped).sub_(0.5).div_(0.5).to(device)
#         ref_lat = self.wan.vae.encode([ref_tensor.unsqueeze(1)])[0].float()
...
#         )
#         ref_shape_ok = (
#             int(ref_lat.shape[0]) == int(target_shape[0]) and
#             int(ref_lat.shape[1]) == 1 and
#             int(ref_lat.shape[2]) == int(target_shape[2]) and
#             int(ref_lat.shape[3]) == int(target_shape[3])
#         )
#         if not ref_shape_ok:
#             raise RuntimeError(
#                 f"legacy_ref_lock 首帧 latent 尺寸不匹配: ref_lat={tuple(ref_lat.shape)} "
...
#         aug_text_len = self._aug_text_len
#         self.wan.model.text_len = aug_text_len
#         self._record_runtime_metric("i2v_legacy_ref_lock_text_len_before", int(self._orig_text_len))
#         self._record_runtime_metric("i2v_legacy_ref_lock_text_len_after", int(self.wan.model.text_len))
#         if self.wan.model.text_len != aug_text_len:
#             raise RuntimeError(
#                 f"[VERIFY] i2v_legacy_ref_lock text_len 设置失败: "
#                 f"current={self.wan.model.text_len}, expected={aug_text_len}"
#             )
...
#                 print(f"[Generate][legacy_ref_lock] 开始 i2v 去噪 ({len(timesteps)} steps)...")
#                 for step_idx, t in enumerate(tqdm(timesteps)):
#                     alpha = self._compute_i2v_ref_blend_alpha(strategy, step_idx, len(timesteps))
#                     if alpha > 0.0:
#                         latents[0][:, :1] = (1.0 - alpha) * latents[0][:, :1] + alpha * ref_lat
...
#                     if alpha > 0.0:
#                         next_latent[:, :1] = (1.0 - alpha) * next_latent[:, :1] + alpha * ref_lat
#                     latents = [next_latent]
...
#         finally:
#             self.wan.model.text_len = self._orig_text_len
#             self._record_runtime_metric("i2v_legacy_ref_lock_text_len_restored", int(self.wan.model.text_len))
#             if self.wan.model.text_len != self._orig_text_len:
#                 raise RuntimeError(
#                     f"[VERIFY] i2v_legacy_ref_lock text_len 未恢复: "
#                     f"current={self.wan.model.text_len}, expected={self._orig_text_len}"
#                 )

#         video = videos[0]
#         if strategy == "hard_lock":
#             exact_ref_frame = ref_tensor
#             try:
#                 if float(video.min().item()) >= -0.1:
#                     exact_ref_frame = ref_tensor.add(1.0).div(2.0)
#             except Exception:
#                 exact_ref_frame = ref_tensor
#             video[:, 0] = exact_ref_frame.to(device=video.device, dtype=video.dtype)
#             self._record_runtime_metric("i2v_legacy_ref_lock_exact_first_frame_replace", 1)
#         else:
#             self._record_runtime_metric("i2v_legacy_ref_lock_exact_first_frame_replace", 0)
...
#             if ref_image is None:
#                 raise ValueError("i2v 模式需要 --ref_image!")
#             if str(getattr(args, "i2v_method", "legacy_ref_lock")).strip().lower() == "animate_ref_slot":
#                 print("[Main][WARN] 当前 inference_metaquery_wan_rms_aligned.py 不支持 animate_ref_slot，自动回退到 legacy_ref_lock。")
#             video = pipeline.generate_i2v(
#                 prompt=args.prompt,
#                 ref_image=ref_image,
#                 negative_prompt=args.negative_prompt,
...
# # 上面的是DiT的注入包括T5和MetaQuery的注入，下面是单纯的MetaQuery注入，适用于任何使用文本编码器的模型
...
# 下面这个是为了适配那个fsdp训练下参数不匹配的请=情况，此时不是首帧强绑定：
# """
# inference_metaquery_wan.py
# ==========================
# MetaQuery + Wan2.2 TI2V 推理脚本。
...
# 下面这个是为了适配那个fsdp训练下参数不匹配的请=情况，此时是首帧强绑定：
"""
inference_metaquery_wan.py
==========================
MetaQuery + Wan2.2 TI2V 推理脚本。
...
def parse_args():
    ...
    p.add_argument(
        "--first_frame_strong_bind",
        action="store_true",
        default=True,
        help="i2v 默认启用首帧强绑定：legacy_ref_lock + hard_lock",
    )
    p.add_argument(
        "--disable_first_frame_strong_bind",
        action="store_false",
        dest="first_frame_strong_bind",
        help="关闭首帧强绑定预设，按 --i2v_method / --i2v_ref_strategy 原值运行",
    )
    p.add_argument("--max_area", type=int, default=480 * 832,
                   help="最大面积")
    p.add_argument("--sampling_steps", type=int, default=50)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.mode == "i2v":
            if ref_image is None:
                raise ValueError("i2v 模式需要 --ref_image!")
            if bool(getattr(args, "first_frame_strong_bind", True)):
                if str(getattr(args, "i2v_method", "legacy_ref_lock")).strip().lower() != "legacy_ref_lock":
                    print(
                        f"[Main][StrongBind] override i2v_method: {args.i2v_method} -> legacy_ref_lock"
                    )
                if str(getattr(args, "i2v_ref_strategy", "hard_lock")).strip().lower() != "hard_lock":
                    print(
                        f"[Main][StrongBind] override i2v_ref_strategy: {args.i2v_ref_strategy} -> hard_lock"
                    )
                args.i2v_method = "legacy_ref_lock"
                args.i2v_ref_strategy = "hard_lock"
                print("[Main][StrongBind] preset active: legacy_ref_lock + hard_lock")
            if str(getattr(args, "i2v_method", "legacy_ref_lock")).strip().lower() == "animate_ref_slot":
                print("[Main][WARN] 当前 inference_metaquery_wan_rms_aligned.py 不支持 animate_ref_slot，自动回退到 legacy_ref_lock。")
            video = pipeline.generate_i2v(
                prompt=args.prompt,
