"""
Determined/torchrun friendly FSDP inference entry for MetaQuery + Wan TI2V.
"""
...
import inference_metaquery_wan as base
from wan_finetune_loader import load_wan_finetune_weights
...
def init_dist(args) -> DistEnv:
    ...
    if enabled and not dist.is_initialized():
        ...
        print(
            ...
            f"backend=nccl timeout_sec={int(timeout.total_seconds())} "
...
class MetaQueryWanPipelineFSDP(base.MetaQueryWanPipeline):
    ...
    def _load_pipeline(self):
        ...
        if self._dist_env.rank == 0:
            print(
                f"[FSDP] TI2V loaded | rank={self._dist_env.rank} "
                f"world_size={self._dist_env.world_size} device={self.args.device} "
                f"dit_fsdp={self.args.dit_fsdp} t5_fsdp={self.args.t5_fsdp} use_sp={self.args.use_sp}"
            )
    ...
    def _maybe_load_wan_finetune_weights(self) -> None:
        ...
        load_wan_finetune_weights(
            model=self.wan.model,
            checkpoint_path_or_dir=self.args.checkpoint_path,
            record_metric=self._record_runtime_metric,
            warn=self._warn,
            rank=self._dist_env.rank,
            enable_fsdp_flat_inference=bool(self.args.dit_fsdp),
        )
...
def main():
    ...
    try:
        ...
        pipeline = MetaQueryWanPipelineFSDP(args, env)
        ...
        print(
            f"[STAGE] rank={env.rank} generate_start mode={args.mode} "
            f"frame_num={args.frame_num} steps={args.sampling_steps}",
            flush=True,
        )
        if args.mode == "i2v":
        ...
        if pipeline is not None and env.rank == 0:
