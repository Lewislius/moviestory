...
def save_mq_checkpoint_bundle(
    ...
    if wan_lora_config:
        trainer_state["wan_lora"] = _to_jsonable(wan_lora_config)
    if extra_info:
    ...
    config_payload = {
        ...
        "wan_trainable_param_count": int(wan_trainable_param_count),
        "has_wan_dit_trainable_pt": bool(len(wan_trainable_state_cpu) > 0),
        "enable_wan_lora": bool(len(wan_lora_state_cpu) > 0),
        "wan_lora_param_count": int(wan_lora_param_count),
        ...
        "wan_train_mode_requested": str(getattr(args, "wan_train_mode", "auto")),
        ...
        "wan_lora_targets": str(getattr(args, "wan_lora_targets", "self_attn,cross_attn,ffn")),
    }
    if wan_lora_config:
...
class MetaQueryWanTrainer:
    ...
    def __init__(self, args):
        ...
        self._wan_lora_config: Dict[str, Any] = {}
        ...
        print(
            f"  Wan LoRA        : enabled={int(bool(args.enable_wan_lora))} "
            f"rank={args.wan_lora_rank} alpha={args.wan_lora_alpha} "
            f"dropout={args.wan_lora_dropout} targets={args.wan_lora_targets}"
        )
        print(
    ...
    def _apply_wan_lora_if_needed(self) -> None:
        wan_model = getattr(self.wan, "model", None)
        if not self._wan_lora_enabled():
            self._wan_lora_module_names = []
            self._wan_lora_config = build_lora_config_dict(
                enabled=False,
                ...
                targets=str(getattr(self.args, "wan_lora_targets", "self_attn,cross_attn,ffn")),
                module_names=[],
            )
            return
        if wan_model is None or not isinstance(wan_model, nn.Module):
            raise RuntimeError("启用 Wan LoRA 失败：wan.model 不可用")
        if bool(getattr(self.args, "dit_fsdp", False)) or bool(getattr(self.args, "use_sp", False)):
            raise RuntimeError("当前 Wan LoRA 仅支持非 FSDP / 非 SP 训练路径，请先使用单进程非分片模式")

        rank = int(getattr(self.args, "wan_lora_rank", 16))
        alpha = float(getattr(self.args, "wan_lora_alpha", float(rank)))
        dropout = float(getattr(self.args, "wan_lora_dropout", 0.0))
        targets = str(getattr(self.args, "wan_lora_targets", "self_attn,cross_attn,ffn"))
        module_names = apply_lora_to_wan_model(
            wan_model,
            rank=rank,
            alpha=alpha,
            dropout=dropout,
        ...
        if not module_names:
            raise RuntimeError(
                "Wan LoRA 已启用，但未匹配到任何目标 Linear。"
                f"请检查 --wan_lora_targets={targets}"
            )
        self._wan_lora_module_names = module_names
        self._wan_lora_config = build_lora_config_dict(
            enabled=True,
            rank=rank,
            alpha=alpha,
        ...
        if self.is_main_process:
            ...
            more = "" if len(module_names) <= 8 else f" ... +{len(module_names) - 8}"
            print(
                f"[WAN-LORA] enabled=1 rank={rank} alpha={alpha} dropout={dropout} "
                f"targets={','.join(self._wan_lora_config.get('targets', []))} "
                f"matched_modules={len(module_names)}"
            )
            print(f"[WAN-LORA] matched preview: {preview}{more}")
    ...
    def _configure_wan_trainable_params(self) -> None:
        ...
        if mode == "full":
            ...
            selected_names = mark_only_lora_trainable(wan_model)
            selected_params = [p for p in wan_model.parameters() if p.requires_grad]
        elif mode == "cond_only":
        ...
        if self.is_main_process:
            ...
            if mode == "lora":
                print(
                    f"[WAN-TRAIN] lora rank={self._wan_lora_config.get('rank', 0)} "
                    f"alpha={self._wan_lora_config.get('alpha', 0.0)} "
                    f"dropout={self._wan_lora_config.get('dropout', 0.0)} "
                    f"targets={','.join(self._wan_lora_config.get('targets', []))}"
                )
            if mode == "cond_only":
                kws = self._wan_cond_keywords()
    ...
    def _load_models(self):
        ...
        self._apply_wan_lora_if_needed()
    ...
    def train(self):
        ...
        dist_enabled = bool(torch.distributed.is_available() and torch.distributed.is_initialized())
        dist_world_size = int(torch.distributed.get_world_size()) if dist_enabled else 1
        dist_rank = int(torch.distributed.get_rank()) if dist_enabled else 0
        sampler = None
        if dist_world_size > 1:
            ...
            if self.is_main_process:
                print(
                    f"[DATALOADER] Using DistributedSampler world_size={dist_world_size} "
                    f"rank={dist_rank} drop_last=False"
                )
    ...
    def _save_checkpoint(self, path, step, extra_info: Dict[str, Any] | None = None):
        ...
        module = self._mq_encoder_module()
        wan_state_cpu, wan_export_info = self._collect_wan_trainable_state_for_checkpoint()
        wan_mode = str(getattr(self, "_effective_wan_train_mode", "frozen")).strip().lower()
        wan_trainable_state_cpu = wan_state_cpu if wan_mode != "lora" else {}
        wan_lora_state_cpu = wan_state_cpu if wan_mode == "lora" else {}
        if not self.is_main_process:
            return
        ckpt_info = save_mq_checkpoint_bundle(
            ...
            args=self.args,
            wan_module=None,
            wan_trainable_state_cpu=wan_trainable_state_cpu,
            wan_lora_state_cpu=wan_lora_state_cpu,
            wan_lora_config=dict(self._wan_lora_config),
            ...
            extra_info={
                ...
                "wan_lora": dict(self._wan_lora_config),
                **wan_export_info,
                **(extra_info or {}),
            },
        )
    ...
    def _collect_wan_trainable_state_for_checkpoint(self) -> Tuple[Dict[str, torch.Tensor], Dict[str, Any]]:
        """
        ...
        if mode == "lora":
            state_cpu = collect_lora_state_dict(wan_model)
            info["wan_export_mode"] = "lora_state"
            ...
            info["wan_lora"] = dict(self._wan_lora_config)
