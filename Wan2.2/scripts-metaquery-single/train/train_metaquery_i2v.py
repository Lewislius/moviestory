...
# 复用 MetaQuery Encoder、数据集与 checkpoint 工具
...
class MetaQueryI2VTrainer:
    ...
    def _init_wandb(self):
        ...
        try:
            ...
            return
        run_name = args.wandb_run_name.strip() or f"wan-i2v-metaquery-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    def train(self):
        ...
        while step < args.num_train_steps:
            ...
            if high_noise_count + low_noise_count > 0:
                total_noise = float(high_noise_count + low_noise_count)
                metrics["train/high_noise_ratio"] = float(high_noise_count / total_noise)
                metrics["train/low_noise_ratio"] = float(low_noise_count / total_noise)
