...
case "${TASK}" in
  ti2v)
    RUN_OUTPUT_DIR="${OUTPUT_ROOT}/ti2v_stage1_openvid_local_steps${NUM_TRAIN_STEPS}"
    ...
    ;;
  i2v)
    RUN_OUTPUT_DIR="${OUTPUT_ROOT}/i2v_stage1_openvid_local_steps${NUM_TRAIN_STEPS}"
    ...
    ;;
  animate)
    RUN_OUTPUT_DIR="${OUTPUT_ROOT}/animate_stage1_openvid_local_steps${NUM_TRAIN_STEPS}"
...
if [[ -n "${RUN_OUTPUT_DIR:-}" ]]; then
  ...
  echo "[VERIFY] before checkpoint: ${RUN_OUTPUT_DIR}/checkpoint-before-training"
  echo "[VERIFY] final checkpoint : ${RUN_OUTPUT_DIR}/checkpoint-final"
  echo "[VERIFY] chain manifest   : ${RUN_OUTPUT_DIR}/training_chain_manifest.json"
  echo "[VERIFY] metrics jsonl    : ${METRICS_JSONL_PATH}"
  echo "[VERIFY] 审计命令示例:"
