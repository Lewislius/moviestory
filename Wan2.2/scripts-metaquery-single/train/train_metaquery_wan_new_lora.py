...
def _load_base_ti2v_module():
    ...
    默认使用 train_metaquery_wan_lora（无 animate-like/hard-lock 专用逻辑）。
    可选通过 WAN_BASE_TI2V_MODULE 显式指定允许模块：
      - train_metaquery_wan_lora
      - train_metaquery_wan
    WAN_BASE_TI2V_FILE 仅作为同名模块的文件路径兜底。
    """
    default_name = "train_metaquery_wan_lora"
    allowed_names = {"train_metaquery_wan", "train_metaquery_wan_lora"}
