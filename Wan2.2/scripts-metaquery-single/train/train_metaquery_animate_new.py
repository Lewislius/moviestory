...
HF_RUNTIME = {}
BASE_ANIMATE = None
DEFAULT_WAN_CKPT = "/home/liuzhirui/model/Wan2.2/Wan2.2-Animate-14B"
...
def parse_runtime_args():
    ...
    parser.add_argument("--output_root", type=str, default="./metaquery_animate_hf_two_stage")
...
def _with_stage_wandb(argv, stage_name):
    ...
    if run_name:
        ...
        out = _set_or_append(out, "--wandb_run_name", f"wan-animate-metaquery-{stage_name}")
...
def _print_plan(runtime, remain):
    ...
    print("[CHECK_ONLY] Animate 两阶段计划")
...
def _init_dist_if_needed(runtime):
    ...
    if enabled:
        print(
            ...
            f"process_device={process_device} "
...
def _run_single_train(runtime, remain):
    ...
    try:
        base_animate = _load_base_animate()
        base_animate.TomAndJerryVideoDataset = HFWanDatasetAdapter
        resolved_argv = _resolve_device_args(runtime, remain, local_rank)
        dit_v = _get_arg_value(resolved_argv, "--dit_device", "unset")
        enc_v = _get_arg_value(resolved_argv, "--encoder_device", "unset")
        print(f"[PIPELINE] resolved_argv={' '.join(map(str, resolved_argv))}", flush=True)
        print(
            f"[TOPO] rank={rank} local_rank={local_rank} world_size={int(os.environ.get('WORLD_SIZE', '1'))} "
            f"gpus_per_process={runtime.gpus_per_process} dit_device={dit_v} encoder_device={enc_v}",
            flush=True,
        )
        if runtime.gpus_per_process > 1 and torch.cuda.is_available():
            try:
                if int(enc_v) >= torch.cuda.device_count() or int(dit_v) >= torch.cuda.device_count():
                    raise RuntimeError(
                        f"设备映射越界: cuda_count={torch.cuda.device_count()} "
                        f"dit_device={dit_v} encoder_device={enc_v}"
                    )
            except ValueError:
                pass
        print(
        ...
        try:
            ...
            trainer = base_animate.MetaQueryAnimateTrainer(args)
            if enabled and rank != 0:
...
def _load_base_animate():
    global BASE_ANIMATE
    if BASE_ANIMATE is None:
        import train_metaquery_animate as base_animate
        BASE_ANIMATE = base_animate
    return BASE_ANIMATE
