...
# Point at the run directory so the Python entrypoint resolves its `latest`
# pointer. CHECKPOINT_DIR may also be a concrete checkpoint-final directory.
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${CODE_ROOT}/checkpoint/three_router_mq256_conn24_strongbind_openvid4000_steps500}"
...
# PROMPT="${PROMPT:-The video shows the woman smiles and waves at the camera.}"
...
# 49 frames matches training. Override with another 4n+1 value (for example
# FRAME_NUM=145) when intentionally generating a longer sequence.
...
OUTPUT_DIR="${OUTPUT_DIR:-${CODE_ROOT}/inference_outputs/openvid4000_3router_strongbind}"
OUTPUT_PATH="${OUTPUT_PATH:-${OUTPUT_DIR}/seed${SEED}_${RUN_TS}.mp4}"
VERIFY_REPORT_PATH="${VERIFY_REPORT_PATH:-${OUTPUT_PATH}.verify.json}"

cmd=(
  python "${CODE_ROOT}/infer_3router_planner_wan_strongbind.py"
...
printf '[RUN] strong_binding=clean_preserved_latent_slot cfg_uncond=empty_mq\n'
printf '[RUN] output=%s\n' "${OUTPUT_PATH}"
