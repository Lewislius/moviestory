...
# Training logs for this checkpoint record FA2.  Pin inference to the same
# backend; otherwise workers with FA3 installed silently choose a different
# kernel and can intermittently return a context-insensitive prediction.
export WAN_FLASH_ATTN_FORCE_VERSION=2
export WAN_FLASH_ATTN_FORCE_CONTIGUOUS=1

CHECKPOINT_DIR="${CHECKPOINT_DIR:-${CODE_ROOT}/checkpoint/three_router_mapped-mq-plus-t5_strongbind_openvid4000_4x48g_steps150/checkpoint-final}"
...
# 49 frames and max_area=262144 exactly match this training run.
...
OUTPUT_DIR="${OUTPUT_DIR:-${CODE_ROOT}/inference_outputs/openvid4000_3router_4x48g_mode1_strongbind}"
OUTPUT_PATH="${OUTPUT_PATH:-${OUTPUT_DIR}/seed${SEED}_${RUN_TS}.mp4}"
VERIFY_REPORT_PATH="${VERIFY_REPORT_PATH:-${OUTPUT_PATH}.verify.json}"

cmd=(
  python "${INFERENCE_ROOT}/infer_3router_wan_4x48g_mode1.py"
...
printf '[RUN] conditioning=mapped_mq(256)+actual_frozen_t5(<=512) wan_text_len=768\n'
