...
class Qwen2Encoder(Qwen2PreTrainedModel):
    supports_gradient_checkpointing = True
    _input_embed_layer = None  # No token embeddings in this encoder
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs=None):
        """
        Override HF default behavior for encoder-only blocks without token embeddings.
        The parent implementation tries to hook input embeddings and emits warnings for this class.
        """
        ...
        self._gradient_checkpointing_func = (
            lambda func, *gc_args: torch.utils.checkpoint.checkpoint(
                func, *gc_args, use_reentrant=use_reentrant
            )
        )
    ...
    def get_input_embeddings(self):
        """This encoder operates on hidden states directly, not token IDs."""
        return None
