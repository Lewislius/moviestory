...
# 这个第二版主要是尝试为了适配图片生成的demo,上面这个第一版是可以适配基于视频的meta-query模型训练的
# Copyright (c) Meta Platforms, Inc. and affiliates.
# All rights reserved.
...
class MetaQuery(PreTrainedModel):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        if "Sana" in config.vae_id:
            self.vae = _hf_from_pretrained_cpu(
                AutoencoderDC, config.vae_id, subfolder="vae"
            )
        else:
            try:
                self.vae = _hf_from_pretrained_cpu(AutoencoderKL, config.vae_id)
            except:
                self.vae = _hf_from_pretrained_cpu(
                    AutoencoderKL, config.vae_id, subfolder="vae"
                )

        if self.loss_type == "flow":
            self.noise_scheduler = _hf_from_pretrained_cpu(
                FlowMatchEulerDiscreteScheduler,
                config.noise_scheduler_id, subfolder="scheduler"
            )
        elif self.loss_type == "diff":
        ...
        self.scheduler = _hf_from_pretrained_cpu(
            DPMSolverMultistepScheduler,
            config.scheduler_id, subfolder="scheduler"
        )
