...
# 环境变量
# ─────────────────────────────────────────────────────────────────────────────
# NCCL 配置（Determined AI 环境关键设置）
export NCCL_TIMEOUT=120          # 30 分钟超时（默认 30 秒太短）
export NCCL_DEBUG=WARN            # 显示 NCCL 警告信息方便排查
export NCCL_IB_DISABLE=1          # 禁用 InfiniBand（Determined 节点内通常用 PCIe/NVLink）
export NCCL_P2P_LEVEL=NVL         # 优先 NVLink P2P 通信
export TORCH_NCCL_BLOCKING_WAIT=0 # 非阻塞等待，减少卡死风险
# export WANDB_MODE=disabled         # 禁用 wandb 避免网络阻塞
