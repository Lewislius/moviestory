...
# # ─── DeepSpeed 兼容性补丁 ───────────────────────────────────────────────────
# # DeepSpeed 0.14+ 新增 count_used_parameters_in_backward，会对所有 requires_grad=True
# # 的参数调用 torch.autograd.graph._get_grad_fn_or_grad_acc(param)。
# # 当模型有部分冻结/解冻的参数（如只解冻 embed_tokens）时，某些参数的 grad_fn 为 None，
# # 导致 AttributeError: 'NoneType' object has no attribute 'next_functions'。
# #
# # 关键：stage_1_and_2.py 通过 from ... import 直接导入了该函数，
# # 所以必须同时 patch ds_utils 模块 和 stage_1_and_2 模块上的引用。
# try:
#     import deepspeed.runtime.utils as ds_utils
#     import deepspeed.runtime.zero.stage_1_and_2 as ds_zero_stage12

#     _orig_count = getattr(ds_utils, 'count_used_parameters_in_backward', None)
#     if _orig_count is not None:
#         def _safe_count_used_parameters_in_backward(all_params):
#             """安全版本：跳过 grad_fn 为 None 的参数"""
#             try:
#                 return _orig_count(all_params)
#             except AttributeError:
#                 return len(all_params)
#         # patch 两个模块上的引用
#         ds_utils.count_used_parameters_in_backward = _safe_count_used_parameters_in_backward
#         if hasattr(ds_zero_stage12, 'count_used_parameters_in_backward'):
...
#     disable_tqdm: bool = False  # 确保显示进度条
...
#         )
#         # Build model normally first (avoids meta device conflict in from_pretrained)
#         model = MetaQuery(
...
#         shard_files = sorted(glob.glob(os.path.join(ckpt_path, "model*.safetensors")))
#         if shard_files:
#             state_dict = {}
#             for shard_file in shard_files:
#                 state_dict.update(safetensors.torch.load_file(shard_file, device="cpu"))
#         else:
#             bin_files = sorted(glob.glob(os.path.join(ckpt_path, "pytorch_model*.bin")))
#             state_dict = {}
...
#     # ─── 训练结束，强制清理并退出 ─────────────────────────────────────────────
#     # wandb 的 atexit handler 可能在网络不好时无限挂起，导致进程无法退出，
#     # 阻塞 shell 脚本进入 Stage 2。这里显式 finish + 超时退出。
#     if trainer.is_world_process_zero():
#         print(f"\n✅ 训练完成! Checkpoint 位于: {training_args.output_dir}")
#     try:
...
if __name__ == "__main__":
    ...
    # ─── 串行化模型加载：rank 0 先加载，其他 rank 等待 ──────────────────────
    # 在 Determined AI 上，多 rank 同时从共享文件系统读取多 GB 模型权重会导致
    # I/O 竞争，部分 rank 极慢，其他 rank 在 NCCL barrier 超时。
    # 让 rank 0 先加载（把文件读入 OS page cache），其他 rank 再加载就能直接命中缓存。
    local_rank = int(os.environ.get("LOCAL_RANK", 0))
    world_size = int(os.environ.get("WORLD_SIZE", 1))

    if world_size > 1:
        import torch.distributed as dist
        # 先绑定 GPU，建立 CUDA context，否则 barrier 不知道用哪块卡通信
        torch.cuda.set_device(local_rank)
        if not dist.is_initialized():
            dist.init_process_group(backend="nccl", device_id=torch.device(f"cuda:{local_rank}"))

    def _barrier():
        """带 device_ids 的 barrier，避免 GPU 映射不明导致挂起"""
        if world_size > 1:
    ...
    if world_size > 1:
        # Rank 0 先加载，将模型文件读入 OS page cache
        if local_rank == 0:
            print("[INFO] Rank 0: 开始加载模型（其他 rank 等待中）...")
            model = _build_model()
            print("[INFO] Rank 0: 模型加载完毕")
        _barrier()
        # 其他 rank 再加载（此时文件已在 page cache 中，速度极快）
        if local_rank != 0:
            print(f"[INFO] Rank {local_rank}: 开始加载模型...")
            model = _build_model()
            print(f"[INFO] Rank {local_rank}: 模型加载完毕")
        _barrier()
    else:
        model = _build_model()
